...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    After clamping to [min, max], values will be normalized to [0, 255].

    Args:
        tensor (Tensor or list[Tensor]): Input tensors.
        rgb2bgr (bool): Whether to change rgb to bgr.
        out_type (numpy type): the numpy type of the returned np array.
        min_max (tuple[int]): min and max values for clamp.
    """
    ...
    for _tensor in tensor:
        ...
        # <<< --- THIS IS THE FIX --- >>>
        # Only perform color conversion if the image is 3-channel and rgb2bgr is True
